...
func broadcaster() {
    ...
    for {
        ...
        case cliSt := <-entering:
            ...
            if len(users) > 0 {
                cliSt.ch <- fmt.Sprintf("%s 也在聊天室中。", strings.Join(users, "; "))
            } else {
...
func handleConn(conn net.Conn) {
	...
	fmt.Fprint(conn, danceImage.AddHead()) // 注意，忽略网络层面的错误
...
func clientRegiste(conn net.Conn) (who string, ok bool) {
    inputFunc := func(sig chan<- struct{}) {
        input := bufio.NewScanner(conn)
        ch := make(chan bool)
        fmt.Fprint(conn, "请给自己取一个好听的网名吧: ") // 注意，忽略网络层面的错误
        for input.Scan() {
            if len(strings.TrimSpace(input.Text())) == 0 { // 禁止发送纯空白字符
                continue
            }
